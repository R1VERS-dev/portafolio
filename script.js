...
// Astronaut character functionality
function initAstronaut() {
    ...
    const messages = [
        ...
        "Codificando desde el cosmos",
        "¡Misión: Programar!",
        "HTML desde el espacio",
        "CSS intergaláctico",
        "JavaScript lunar",
        "¡Explorador digital!",
        "De la Tierra al código",
        "¡Astronauta programador!",
        "Navegando por el portafolio",
        "¡Despegando con proyectos!",
        "Orbitando en desarrollo",
        "¡Aterrizando en soluciones!",
        "Conectando mundos digitales",
        "¡Transmisión desde la ISS!",
        "Control de misión: ¡Todo OK!",
        "Servidor en órbita",
        "¡Base espacial activa!"
    ];
    ...
    function cycleSpeechMessages() {
        showSpeechBubble(messages[currentMessageIndex]);
    ...
    function triggerWave() {
        ...
        isWaving = true;
        rightArm.style.animation = 'waveHand 1s ease-in-out 3';
        
        setTimeout(() => {
            rightArm.style.animation = 'armFloat 3s ease-in-out infinite 0.5s';
            isWaving = false;
        }, 3000);
    }
    
    // Eye tracking (simple left/right movement)
    function updateEyeDirection() {
        const eyes = document.querySelectorAll('.eye');
        const directions = ['translateX(-1px)', 'translateX(0px)', 'translateX(1px)'];
    ...
    // Initialize astronaut behavior
    function startAstronautBehavior() {
        // Initial welcome message
        setTimeout(() => {
            showSpeechBubble("¡Hola! Soy Gustavo");
            triggerWave();
        }, 1000);
        
        // Start cycling messages every 8 seconds
        speechInterval = setInterval(cycleSpeechMessages, 8000);
        
        // Random eye movement
        setInterval(updateEyeDirection, 4000);
    ...
    astronaut.addEventListener('click', function() {
        triggerWave();
        showSpeechBubble("¡Gracias por hacer clic!");
    });
    
    // Hover interaction
    astronaut.addEventListener('mouseenter', function() {
        if (!speechBubble.classList.contains('show')) {
            showSpeechBubble("👋 ¡Hola!");
        }
        triggerWave();
    });
    
    // Start the behavior
    startAstronautBehavior();
    
    // Pause speech when page is not visible
